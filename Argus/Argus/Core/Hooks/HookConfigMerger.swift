...
struct HookConfigMerger {
    static let bridgeCommand = "~/.argus/bin/argus-bridge"
    static let marker = "argus-bridge"
    ...
    static func merge(into existingJSON: [String: Any], for agent: AgentSource) -> [String: Any] {
        ...
        for (internalEvent, nativeEvent) in agent.eventMapping {
            ...
            if let existingIndex = findArgusHookIndex(in: eventArray, format: agent.hookFormat) {
    ...
    static func remove(from existingJSON: [String: Any], for agent: AgentSource) -> [String: Any] {
        ...
        for (_, nativeEvent) in agent.eventMapping {
            ...
            eventArray.removeAll { entry in
                containsArgus(entry: entry, format: agent.hookFormat)
            }
    ...
    static func hasArgusHooks(in json: [String: Any], for agent: AgentSource) -> Bool {
        ...
        for (_, value) in hooks {
            ...
            for entry in eventArray {
                if containsArgus(entry: entry, format: agent.hookFormat) {
                    return true
                }
            }
        }
    ...
    private static func findArgusHookIndex(in array: [[String: Any]], format: HookFormat) -> Int? {
        array.firstIndex { containsArgus(entry: $0, format: format) }
    }

    private static func containsArgus(entry: [String: Any], format: HookFormat) -> Bool {
        switch format {
        case .claude, .nested:
