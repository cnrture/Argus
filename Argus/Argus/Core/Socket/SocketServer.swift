...
final class SocketServer {
    ...
    private let queue = DispatchQueue(label: "com.argus.socket", qos: .userInitiated)

    weak var delegate: SocketServerDelegate?

    init() {
        let dir = NSHomeDirectory() + "/.argus"
        socketPath = dir + "/argus.sock"
        ensureDirectory(dir)
    }
    ...
    private func acceptConnection() {
        ...
        let clientQueue = DispatchQueue(label: "com.argus.client.\(clientFd)", qos: .userInitiated)
    ...
    private func processLine(_ line: String, clientFd: Int32) {
        ...
        // let logLine = "[Argus] \(line.prefix(500))\n"
        // if let d = logLine.data(using: .utf8) { let p = NSHomeDirectory() + "/.argus/debug.log"; if FileManager.default.fileExists(atPath: p) { if let fh = FileHandle(forWritingAtPath: p) { fh.seekToEndOfFile(); fh.write(d); fh.closeFile() } } else { FileManager.default.createFile(atPath: p, contents: d) } }
        do {
            ...
            print("[Argus] Parse error: \(error) for line: \(line.prefix(200))")
        }
    }
    ...
    private func sendResponse(_ response: SocketResponse, to fd: Int32) {
        ...
        do {
            ...
            print("[Argus] Response encode error: \(error)")
        }
    }
}
